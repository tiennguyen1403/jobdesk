...
import {
  disconnectFreelancer,
  freelancerConnectUrl,
  freelancerStatusQueryKey,
  getFreelancerStatus,
  type FreelancerStatusResult,
} from '../../lib/api/freelancer'

// A deliberate sibling of UpworkPanel: same connect / status / disconnect shape,
// so the two providers read identically on the Sources page. Kept as its own
// component (not a shared abstraction) so the panels never collide on merge.
...
export default function FreelancerPanel() {
  ...
  const { data: status, isLoading, isError } = useQuery({
    queryKey: freelancerStatusQueryKey(),
  ...
  const disconnect = useMutation({
    mutationFn: disconnectFreelancer,
    onSuccess: (result) => queryClient.setQueryData(freelancerStatusQueryKey(), result),
  })
  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900 p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold">Freelancer</h2>
...
function NotConfigured() {
  return (
    <div className="rounded-lg border border-dashed border-slate-700 bg-slate-950/40 p-4">
      <p className="text-sm text-slate-300">Freelancer integration is not configured.</p>
      <p className="mt-1 text-xs text-slate-500">
        Set <code className="font-mono text-slate-400">FREELANCER_CLIENT_ID</code> and{' '}
        <code className="font-mono text-slate-400">FREELANCER_CLIENT_SECRET</code> in{' '}
...
function Disconnected() {
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <a href={freelancerConnectUrl()} className={connectClass}>
        Connect Freelancer
      </a>
    </div>
  )
